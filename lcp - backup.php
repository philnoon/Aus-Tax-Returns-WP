	...
	class lead_capture_pro
	{
		...
		*/
		...
		function register_css($type='admin')
		{
			...
			if($type=='admin'){
				wp_register_style( 'lcp-admin-styles', $this->settings['url'] .'/css/lead-capture-pro-admin.css' );
                                wp_register_style( 'lcp-admin-styles', $this->settings['url'] .'/css/bootstrap-responsive.css' );
                                wp_register_style( 'lcp-admin-styles', $this->settings['url'] .'/css/bootstrap.css' );                                
		...
		function register_js($type='admin')
		{
			...
			if($type=='admin'){
				...
				// none currently
                                //wp_enqueue_script( 'lcp-admin', site_url() .'/wp-content/bootstrap-2.3.2/assets/js/bootstrap.min.js' );
                                //wp_enqueue_script( 'lcp-admin', site_url() .'/wp-content/bootstrap-2.3.2/assets/js/bootstrap-modal.js' );
			}
		...
		*/
		...
		function admin_dashboard()
		{
			...
			$company_results 	= $this->companies->get('pending');
		 ...
		 *
		 */
		function lcp_admin_company_details($id)
		{

			$company = $this->companies->byCompanyId($id);
		...
		/*
		 *
		 * 	Purpose: Displays the leads overview page
		 * 	Added in Version 1.0
		 *
		 */
		function leads_overview()
		{
			...
			$page_title = "Lodgements Available";
		...
		function lead_signup_form_shortcode (){
			...
			if ( count($_POST)>0) {
				...
				if (true == $validated) {
					...
					$company_data['salutation_id'] = $_POST['title'];
					$company_data['first_name'] = $_POST['first_name'];
					$company_data['lastname'] = $_POST['lastname'];
					$company_data['company_number'] = $_POST['company_number'];
					$company_data['address'] = $_POST['address'];
					$company_data['address2'] = $_POST['address2'];
					$company_data['town'] = $_POST['town'];
					$company_data['county'] = $_POST['county'];
					$company_data['postcode'] = $_POST['postcode'];
					$company_data['company_description'] = $_POST['company_description'];
					$company_data['phone'] = $_POST['phone'];
					$company_data['alt_phone'] = $_POST['alt_phone'];
					$company_data['website'] = $_POST['website'];
					$company_data['service_range'] = $_POST['service_range'];
					$company_data['receive_leads'] = $_POST['receive_leads'];
                                        */
                                        $company_data['first_name'] = $_POST['first_name'];
					...
					if(! is_wp_error($user_id)){
						...
						//$email_data['site_name']		= get_bloginfo('name');
						$email_data['first_name']		= $_POST['first_name'];
						//$email_data['last_name']		= $_POST['firstname'];
						//$email_data['company_name'] 	= $_POST['company_name'];
						...
						$broadcast->send('company-thanks', $email_data, $_POST['email'], 'Thanks for sigining-up');
                                                ...
                                                //$results = $this->companies->byCompanyID($lastId);                                                      
